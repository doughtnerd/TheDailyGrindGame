...
[CustomEditor(typeof(LoadingScreenProBase))]
[CanEditMultipleObjects]
public class LoadingScreenProDrawer : Editor {
    protected LoadingScreenProBase loadingScreenPro;
    ...
    public override void OnInspectorGUI() {
        loadingScreenPro = target as LoadingScreenProBase;

        if (loadingScreenPro == null)
            return;
    ...
    protected virtual void DrawFields() {
        ...
        if (loadingScreenPro.config != null && loadingScreenPro.config.loadAdditively) {
        ...
        GUI.enabled = (loadingScreenPro.behaviorAfterLoad == LoadingScreenProBase.BehaviorAfterLoad.ContinueAutomatically);
    ...
    void DrawHelpBox() {
        if (IsCurrentSceneLoadingScene() == false) 
            EditorGUILayout.HelpBox("This scene is not set to the LoadingScreenConfig script!\nIt is currently set to: \"" + loadingScreenPro.GetLoadingSceneName() +
                "\".\nEdit the script where it says !!!EDIT HERE!!!", MessageType.Warning);
    ...
    bool HasLegalConfig() {
        return loadingScreenPro.config != null;
    }
    ...
    void SetLegalConfig() {
        ...
        Undo.RecordObject(loadingScreenPro, "Detected Config File");
        loadingScreenPro.config = config;
        EditorUtility.SetDirty(loadingScreenPro);
    }


    bool IsCurrentSceneLoadingScene() {
        var name = loadingScreenPro.GetLoadingSceneName();
        Scene scene;
    ...
    void TriggerAudioDetection() {
        ...
        Undo.RecordObject(loadingScreenPro, "Detected AudioListener");
        loadingScreenPro.DetectAudioListener();
        EditorUtility.SetDirty(loadingScreenPro);
    }
    ...
    void TriggerViewDetection() {
        ...
        Undo.RecordObject(loadingScreenPro, "Detected Views in Active Objects");
